          ...
          <div className="flex gap-1 bg-white/5 rounded-xl p-1 border border-white/8 max-w-4xl">
            {tabs.map(tab => (
              <button
                ...
                className={`flex-1 py-2.5 px-2 rounded-lg text-[11px] font-bold uppercase tracking-widest transition-all duration-200 flex items-center justify-center gap-1.5 ${
                  activeTab === tab.id
              ...
              >
                {tab.label}
                <span className={`min-w-[18px] h-[18px] px-1 rounded-full text-[9px] flex items-center justify-center font-black ${
                  activeTab === tab.id ? 'bg-black/20 text-black' : 'bg-white/8 text-[#555]'
                }`}>{tab.count}</span>
              </button>
            ))}
          </div>
            ...
            {(workflow.tools ?? []).map((tool, i) => (
              ...
              >
                <div className="flex items-start justify-between mb-2 gap-2">
                  ...
                  <span className={`text-[10px] px-2.5 py-1 rounded-full font-bold uppercase tracking-widest flex-shrink-0 ${
                    tool.free
              ...
              <div key={`prompt-${i}`} className="card-glass p-6">
                <div className="flex items-start justify-between mb-4 gap-4">
                  ...
                  <button
                    ...
                    className={`flex-shrink-0 text-[10px] px-3 py-2 rounded-lg border transition-all uppercase tracking-widest font-bold ${
                      copied === `p${i}`
